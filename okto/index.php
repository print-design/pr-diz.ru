            ...
            <div class="wrapper" style="position: absolute; top: 100px; bottom: 0; left: 0; right: 0; padding-left: 75px;">
                <nav id="sidebar">
                    <div id="contacts" style="overflow: auto; position: absolute; top: 0px; bottom: 0; left: 0; right: 15px;">
                        <?php
                        $me = GetUserId();
                        $sql = "select id, last_name, first_name from user where active = 1 and id <> $me order by last_name, first_name";
                        $fetcher = new Fetcher($sql);
                        while($row = $fetcher->Fetch()):
                        ?>
                        <button type="button" class="btn btn-light w-100 mt-1 mb-1 btn_contact" style="text-align: left;" data-id="<?=$row['id'] ?>"><?=$row['last_name'].' '.$row['first_name'] ?></button>
                        <?php endwhile; ?>
                    </div>
                </nav>

...
if(null !== filter_input(INPUT_POST, 'norm_gap_submit')) {
    ...
    if($form_valid) {
        ...
        $sql = "select gap_raport, gap_stream from norm_gap where machine_id = $machine_id order by date desc limit 1";
        ...
        if($old_gap_raport != $new_gap_raport || $old_gap_stream != $new_gap_stream) {
            $sql = "insert into norm_gap (machine_id, gap_raport, gap_stream) values ($machine_id, $new_gap_raport, $new_gap_stream)";

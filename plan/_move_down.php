...
if($shift == 'day') {
    $sql = "select pp.id, pp.date, pp.shift "
            . "from plan_part pp "
            . "inner join calculation c on pp.calculation_id = c.id "
            . "where c.machine_id = $machine_id and pp.date >= '$date'";
}
elseif($shift == 'night') {
    $sql = "select pp.id, pp.date, pp.shift "
            . "from plan_part pp "
            . "inner join calculation c on pp.calculation_id = c.id "
            . "where c.machine_id = $machine_id and pp.date = '$date' and pp.shift = 'night' "
            . "union "
            . "select pp.id, pp.date, pp.shift "
            . "from plan_part pp "
            . "inner join calculation c on pp.calculation_id = c.id "
            . "where c.machine_id = $machine_id and pp.date > '$date'";
}
...
foreach($rows as $row) {
    ...
    if($row['shift'] == 'day') {
        $sql = "update plan_part set shift = 'night' where id = ".$row['id'];
    }
    elseif($row['shift'] == 'night') {
        $sql = "update plan_part set shift = 'day', date = date_add(date, interval 1 day) where id = ".$row['id'];
    }
...
foreach($rows as $row) {
    ...
    if($row['shift'] == 'day') {
        $sql = "update plan_part_continuation set shift = 'night' where id = ".$row['id'];
    }
    elseif($row['shift'] == 'night') {
        $sql = "update plan_part_continuation set shift = 'day', date = date_add(date, interval 1 day) where id = ".$row['id'];
    }

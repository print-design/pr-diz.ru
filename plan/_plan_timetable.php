...
class PlanTimetable {
    ...
    public function __construct($work_id, $machine_id, $dateFrom, $dateTo) {
        ...
        $sql = "select e.id id, e.date, e.shift, ".PLAN_TYPE_EDITION." as type, if(isnull(e.worktime_continued), 0, 1) as has_continuation, ifnull(e.worktime_continued, e.worktime) worktime, e.position, c.id calculation_id, c.name calculation, c.raport, c.lamination_roller_width, c.length, c.stream_width, c.streams_number, c.ink_number, c.unit, c.quantity, "
                ...
                . "union "
                . "select ev.id, ev.date, ev.shift, ".PLAN_TYPE_EVENT." as type, 0 as has_continuation, ev.worktime, ev.position, ev.id calculation_id, ev.text calculation, 0 as raport, 0 as lamination_roller_width, 0 as length, 0 as stream_width, 0 as streams_number, 0 as ink_number, '' as unit, 0 as quantity, "
                . "0 as quantity_sum, "
                . "0 as gap_raport, "
                . "0 as length_pure_1, "
                . "0 as length_pure_2, "
                ...
                . "union "
                . "select pc.id, pc.date, pc.shift, ".PLAN_TYPE_CONTINUATION." as type, pc.has_continuation, pc.worktime, 1 as position, c.id calculation_id, c.name calculation, c.raport, c.lamination_roller_width, c.length, c.stream_width, c.streams_number, c.ink_number, c.unit, c.quantity, "
                . "(select sum(quantity) from calculation_quantity where calculation_id = c.id) quantity_sum, "
                . "(select gap_raport from norm_gap where date <= c.date order by id desc limit 1) as gap_raport, "

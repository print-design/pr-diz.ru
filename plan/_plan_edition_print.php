<tr>
            ...
            for($i = 1; $i <= $ink_number; $i++) {
                ...
                switch($ink_i) {
                    ...
                    case INK_PANTON:
                        ...
                        $color_weight = $print_area * $ink_expense * $percent_i / 1000 / 100;
                        array_push($color_lines, "<span class='text-nowrap'>P".$color_i." - ".DisplayNumber($color_weight, 2)." кг</span>");
                        break;
                    case INK_WHITE:
                    ...
                    case INK_LACQUER:
                        switch($lacquer_i) {
                            ...
                            case LACQUER_SELECTIVE:
                                ...
                                $color_weight = $print_area * $ink_expense * $percent_i / 1000 / 100;
                                array_push($color_lines, "<span class='text-nowrap'>Лак выборочный - ".DisplayNumber($color_weight, 2)." кг</span>");
                                break;
                        }
                        break;
                }
    ...
    <td class="<?=$top.' '.$this->plan_shift->shift ?>">
        ...
        if(!empty($this->edition['status_id']) && !$this->edition['has_continuation']) {
            $this->ShowOrderStatusPrint($this->edition['status_id'], $this->edition['length_cut'], $this->edition['weight_cut'], $this->edition['quantity_sum'], $this->edition['quantity'], $this->edition['unit'], $this->edition['raport'], $this->edition['length'], $this->edition['gap_raport'], $this->edition['status_comment']);
        }
        ?>
    </td>

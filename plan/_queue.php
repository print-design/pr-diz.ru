...
class Queue {
    ...
    private function ShowLaminate() {
        $sql = "select ".TYPE_EVENT." as type, 1 as position, id, 0 as calculation_id, text calculation, 0 as work_type_id, '' customer, 0 length, 0 ink_number, 0.0 raport, now() as status_date, "
                ...
                . "inner join calculation_result cr on cr.calculation_id = c.id "
                . "inner join user u on c.manager_id = u.id "
                . "left join plan_part peprintpart on peprintpart.calculation_id = c.id and peprintpart.work_id = ".WORK_PRINTING." and (select count(id) from plan_part where calculation_id = peprintpart.calculation_id and work_id = peprintpart.work_id and id < peprintpart.id) = 0 "
    ...
    private function ShowCut() {
        ...
        $sql .= " union "
                ...
                . "left join plan_part peprintpart on peprintpart.calculation_id = c.id and peprintpart.work_id = ".WORK_PRINTING." and (select count(id) from plan_part where calculation_id = peprintpart.calculation_id and work_id = peprintpart.work_id and id < peprintpart.id) = 0 "
                . "left join plan_edition peprint on peprint.calculation_id = c.id and peprint.work_id = ".WORK_PRINTING." "
                . "left join plan_part pelampart on pelampart.calculation_id = c.id and pelampart.work_id = ".WORK_LAMINATION." and (select count(id) from plan_part where calculation_id = pelampart.calculation_id and work_id = pelampart.work_id and id < pelampart.id) = 0 "

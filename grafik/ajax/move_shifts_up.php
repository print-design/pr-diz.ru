...
if($shift_from == 'day') {
    //if($half == 'true') {
    //    $sql = "update workshift set date = if(shift = 'day', date_add(date, interval -$count_1 day), date_add(date, interval -$count day)), shift = if(shift = 'day', 'night', 'day') where machine_id = $machine_id and date >= '$from'$where_to";
    ...
    //if($half == 'true') {
    //    $sql = "update workshift set date = if(shift = 'day', date_add(date, interval -$count_1 day), date_add(date, interval -$count day)), shift = if(shift = 'day', 'night', 'day') where machine_id = $machine_id and (date > '$from' or (date = '$from' and shift = 'night'))$where_to";
    //    $error_message = (new Executer($sql))->error;
    //}
    //else {

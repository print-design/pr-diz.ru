        ...
        if($form_valid) {
            ...
            if(empty($error_message)) {
                if(!empty($row) && key_exists('status_id', $row) && $row['status_id'] == ROLL_STATUS_UTILIZED) {
                    header('Location: '.APPLICATION.'/roll/'.BuildQueryAddRemove('status', ROLL_STATUS_UTILIZED, 'id'));
                }
                elseif(!empty ($row) && key_exists('status_id', $row) && $row['status_id'] == ROLL_STATUS_CUT) {
                    header('Location: '.APPLICATION.'/roll/'.BuildQueryAddRemove('status', ROLL_STATUS_CUT, 'id'));
                }
                else {
...
<html lang="ru">
    ...
    <body>
        ...
        <div class="container-fluid">
            ...
            ?>
            <a class="btn btn-light backlink" href="<?=APPLICATION ?>/roll/<?= BuildQueryAddRemove('status', ROLL_STATUS_UTILIZED, 'id') ?>">Назад</a>
            <?php elseif (isset($status_id) && $status_id == ROLL_STATUS_CUT): ?>
            <a class="btn btn-light backlink" href="<?=APPLICATION ?>/roll/<?= BuildQueryAddRemove('status', ROLL_STATUS_CUT, 'id') ?>">Назад</a>
            <?php else: ?>

...
function Filter() {
    ...
    if(!empty($thickness)) {
        $result .= " and p.thickness = ".$thickness;
    }
    
    $width_from = filter_input(INPUT_GET, 'width_from');
    if(!empty($width_from)) {
        $result .= " and p.width >= $width_from";
    }
    
    $width_to = filter_input(INPUT_GET, 'width_to');
    if(!empty($width_to)) {
        $result .= " and p.width <= $width_to";
    }
    ...
    if(!empty($find)) {
        $result .= " and (p.id='$find' or p.id='$findtrim' or p.cell='$find' or p.comment like '%$find%')";
    }
...
$sql = "select sum(pr.weight) total_weight "
        ...
        . "where (prsh.status_id is null or prsh.status_id = $free_status_id)".Filter();

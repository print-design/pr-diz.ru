...
$sql = "select cts.id, cts.printed, pe.machine_id "
        . "from calculation_take_stream cts "
        . "inner join calculation_take ct on cts.calculation_take_id = ct.id "
        . "inner join plan_edition pe on pe.calculation_id = ct.calculation_id "
        . "where cts.plan_employee_id is null and cts.id not in (select id from temp_calculation_take_stream)";
...
if($row = $fetcher->Fetch()) {
    ...
    if(empty($employee_id)) {
        $sql = "insert into temp_calculation_take_stream (id) values ($id)";

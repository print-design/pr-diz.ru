...
// Авторизация
if(!IsInRole(CUTTER_USERS) && !IsInRole(array(ROLE_NAMES[ROLE_TECHNOLOGIST], ROLE_NAMES[ROLE_SCHEDULER], ROLE_NAMES[ROLE_LAM_HEAD]))) {
...
if(null !== filter_input(INPUT_POST, 'stream_print_submit')) {
    ...
    if(empty($weight) && empty($length) && empty($radius)) {
        $is_valid = true;
        $validation1 = true;
        $validation2 = true;
    }
            ...
            <div class="row">
                <div class="col-8">
                    ...
                    <div class="d-flex justify-content-xl-start mb-4" id="calculation_streams_bottom" data-id="0" ondragover="DragOverBottom(event);" ondrop="DropBottom(event);">
                        ...
                        ?>
                        <div><a href="finished.php?id=<?=$id ?>&machine_id=<?=$machine_id ?>" class="btn btn-light pl-4 pr-4 mr-4<?=$finished_class ?>"><i class="fas fa-check mr-2"></i>Тираж выполнен</a></div>

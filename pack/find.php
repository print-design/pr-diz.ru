...
<form class="form-inline ml-auto mr-3<?=$find_class ?>" method="get" id="find-form" action="<?=APPLICATION.'/pack/' ?>">
    <div class="input-group input-group-sm<?=$group_class ?>" id="find-group">
        ...
        <?= filter_input(INPUT_GET, "find") ?>
            &nbsp;&nbsp;
            <a href="<?=APPLICATION.'/pack/'.(filter_input(INPUT_GET, 'status_id') === null ? "" : "?status_id=". filter_input(INPUT_GET, 'status_id')) ?>"><i class="fas fa-times" style="color: white;"></i></a>
        </div>
    </div>
</form>
<?php

...
$extracharge = null; $usd = null; $euro = null; $cost = null; $cost_per_unit = null; $shipping_cost = null; $shipping_cost_per_unit = null; $cliche_price = null; $total_weight_dirty = null;
...
if(!empty($id)) {
    $sql_calculation_result = "select extracharge, usd, euro, cost, cost_per_unit, shipping_cost, shipping_cost_per_unit, cliche_price, total_weight_dirty, "
    ...
    if($row = $fetcher->Fetch()) {
        $extracharge = $row['extracharge']; $usd = $row['usd']; $euro = $row['euro']; $cost = $row['cost']; $cost_per_unit = $row['cost_per_unit']; $shipping_cost = $row['shipping_cost']; $shipping_cost_per_unit = $row['shipping_cost_per_unit']; $cliche_price = $row['cliche_price']; $total_weight_dirty = $row['total_weight_dirty'];
        ...
        $film_cost = $calculation->film_price_1 + $calculation->film_price_2 + $calculation->film_price_3;
        $work_cost = $calculation->work_price_1 + $calculation->work_price_2 + $calculation->work_price_3;
        $ink_cost = 0;
        ...
        $new_cliche_price = 0;
        for($i=1; $i<=$param_ink_number; $i++) {
            $new_cliche_price += $calculation->cliche_prices[$i];
        }
        if($new_cliche_price == null) $new_cliche_price = "NULL";
        ...
        $new_film_price = $calculation->film_price_1;
        if($new_film_price === null) $new_film_price = "NULL";
    
        // Основная плёнка цена за кг = стоимость основной плёнки / вес
        $new_film_price_per_unit = $calculation->film_price_1 / $calculation->weight_dirty_1;
        ...
        $new_lamination1_film_price = $calculation->film_price_2;
        if($new_lamination1_film_price === null) $new_lamination1_film_price = "NULL";
    
        // Лам 1 цена за кг = лам 1 цена / вес
        $new_lamination1_film_price_per_unit = empty($calculation->weight_dirty_2) ? 0 : $calculation->film_price_2 / $calculation->weight_dirty_2;
        ...
        // Лам 2 плёнка цена
        $new_lamination2_film_price = $calculation->film_price_3;
        if($new_lamination2_film_price === null) $new_lamination2_film_price = "NULL";
    
        // Лам 2 цена за кг = лам 2 плёнка цена / вес
        $new_lamination2_film_price_per_unit = empty($calculation->weight_dirty_3) ? 0 : $calculation->film_price_3 / $calculation->weight_dirty_3;
        ...
        $ink_price = 0;
        for($i=1; $i<=$param_ink_number; $i++) {
            $ink_price += $calculation->ink_prices[$i];
        }
        $new_ink_price = null;
        ...
        $new_work_price = $calculation->work_price_1;
        ...
        // Работа лам 1, руб
        $new_lamination1_work_price = $calculation->work_price_2;
        ...
        // Работа лам 2, руб
        $new_lamination2_work_price = $calculation->work_price_3;
        ...
        if(empty($error_message)) {
            $sql = "insert into calculation_result (calculation_id, extracharge, usd, euro, cost, cost_per_unit, shipping_cost, shipping_cost_per_unit, cliche_price, total_weight_dirty, "
                    . "film_price, film_price_per_unit, width, weight_pure, length_pure, weight_dirty, length_dirty, "
                    . "lamination1_film_price, lamination1_film_price_per_unit, lamination1_width, lamination1_weight_pure, lamination1_length_pure, lamination1_weight_dirty, lamination1_length_dirty, "
                    . "lamination2_film_price, lamination2_film_price_per_unit, lamination2_width, lamination2_weight_pure, lamination2_length_pure, lamination2_weight_dirty, lamination2_length_dirty, "
                    . "film_waste_price, film_waste, ink_price, ink_weight, work_price, work_time, "
                    . "lamination1_film_waste_price, lamination1_film_waste, glue_price1, glue_expense1, lamination1_work_price, lamination1_work_time, "
                    . "lamination2_film_waste_price, lamination2_film_waste, glue_price2, glue_expense2, lamination2_work_price, lamination2_work_time) "
                    . "values ($id, $new_extracharge, $new_usd, $new_euro, $new_cost, $new_cost_per_unit, $new_shipping_cost, $new_shipping_cost_per_unit, $new_cliche_price, $new_total_weight_dirty, "
...
<div id="calculation"<?=$calculation_class ?> style="position: absolute; bottom: auto; right: 10px; margin-top: 60px;">
    ...
    <div class="row text-nowrap">
        <div class="col-12">
            ...
            <div class="value mb-2"><?= Display(floatval($cliche_price), 0) ?> &#8381;&nbsp;&nbsp;&nbsp;<span style="font-weight: normal;" id="right_panel_new_forms"><?=$new_forms_number ?>&nbsp;шт&nbsp;<?= Display($stream_width * $streams_number + ($ski == NO_SKI ? 0 : 20), 0) ?>&nbsp;мм&nbsp;<i class="fas fa-times" style="font-size: small;"></i>&nbsp;<?= (intval($raport) + 20) ?>&nbsp;мм</span></div>    
        </div>
    </div>
    <?php endif; ?>
    <div class="mt-3">
    ...
    <div class="row text-nowrap">
        <div class="col-4 pr-4">
            ...
            <div class="value mb-2"><?= Display(floatval($film_price), 0) ?> &#8381;&nbsp;&nbsp;&nbsp;<span style="font-weight: normal;"><?= Display(floatval($film_price_per_unit), 3) ?> &#8381; за кг</span></div>
            <div>Ширина</div>
        ...
        <div class="col-4 pr-4" style="border-left: solid 2px #ced4da;">
            ...
            <div>Закупочная стоимость</div>
            <div class="value mb-2"><?= Display(floatval($lamination1_film_price), 0) ?> &#8381;&nbsp;&nbsp;&nbsp;<span style="font-weight: normal;"><?= Display(floatval($lamination1_film_price_per_unit), 3) ?> &#8381; за кг</span></div>
            <div>Ширина</div>
        ...
        <div class="col-4 pr-4" style="border-left: solid 2px #ced4da;">
            ...
            <div>Закупочная стоимость</div>
            <div class="value mb-2"><?= Display(floatval($lamination2_film_price), 0) ?> &#8381;&nbsp;&nbsp;&nbsp;<span style="font-weight: normal;"><?= Display(floatval($lamination2_film_price_per_unit), 3) ?> &#8381; за кг</span></div>
            <div>Ширина</div>

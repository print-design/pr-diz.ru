...
if(!empty($id)) {
    ...
    if($row = $fetcher->Fetch()) {
        ...
        $param_laminator = null;
        $param_laminator_id = null;
        ...
        $param_extra_expense = null; // Дополнительные расходы с кг/шт
        ...
        if ($row = $fetcher->Fetch()) {
            ...
            $param_laminator = $row['laminator'];
            $param_laminator_id = $row['laminator_id'];
            ...
            $param_lacquer_1 = $row['lacquer_1']; $param_lacquer_2 = $row['lacquer_2']; $param_lacquer_3 = $row['lacquer_3']; $param_lacquer_4 = $row['lacquer_4']; $param_lacquer_5 = $row['lacquer_5']; $param_lacquer_6 = $row['lacquer_6']; $param_lacquer_7 = $row['lacquer_7']; $param_lacquer_8 = $row['lacquer_8'];
        ...
        if(empty($error_message)) {
            if(empty($param_machine_id)) {
                ...
                $sql = "select time, length, stamp, waste_percent from norm_priladka where date <= '$param_date' and machine_id = $param_machine_id order by id desc limit 1";
                $fetcher = new Fetcher($sql);
                if ($row = $fetcher->Fetch()) {
            ...
            if(empty($param_laminator_id)) {
                ...
                $sql = "select time, length, waste_percent from norm_laminator_priladka where date <= '$param_date' and laminator_id = $param_laminator_id order by id desc limit 1";
                $fetcher = new Fetcher($sql);
                if($row = $fetcher->Fetch()) {
            ...
            if(empty($param_laminator_id)) {
                ...
                $sql = "select glue_price, glue_currency, glue_expense, glue_expense_pet, solvent_price, solvent_currency, solvent_part "
                        . "from norm_glue where date <= '$param_date' and laminator_id = $param_laminator_id order by id desc limit 1";
                $fetcher = new Fetcher($sql);
                if($row = $fetcher->Fetch()) {
        ...
        $calculation = new Calculation($data_priladka, $data_priladka_laminator, $data_machine, $data_machine_laminator, $data_ink, $data_glue, $data_cliche, $data_extracharge, $new_usd, $new_euro, 
                ...
                $param_cliche_in_price, $param_extracharge, $param_extracharge_cliche, $param_customer_pays_for_cliche, $param_extra_expense);

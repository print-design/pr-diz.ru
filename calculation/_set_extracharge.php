<?php
include '../include/topscripts.php';
include './calculation.php';

$work_type_id = filter_input(INPUT_GET, 'work_type_id');
$extracharge = filter_input(INPUT_GET, 'extracharge');
$result = array();

if(empty($work_type_id)) {
    $result['error'] = "Не указан тип работы";
}
elseif(empty ($extracharge)) {
    $result['error'] = "Не указан размер наценки";
}
else {
    $result['error'] = '';
    $result['shipping_cost'] = $extracharge." ".$work_type_id;
    $result['shipping_cost_per_unit'] = $extracharge." ".$work_type_id;
    $result['income'] = $extracharge." ".$work_type_id;
    $result['income_per_unit'] = $extracharge." ".$work_type_id;
}

echo json_encode($result);
?>
<?php
include '../include/topscripts.php';
include './calculation.php';

$id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
$extracharge_knife = filter_input(INPUT_GET, 'extracharge_knife');
$result = array();

if(empty($id)) {
    $result['error'] = "Не указан ID расчёта";
}
elseif($extracharge_knife === null || $extracharge_knife === '') {
    $result['error'] = "Не указан размер наценки";
}
else {
    $sql = "update calculation set extracharge_knife=? where id=?";
    $executer = new Executer($sql, [$extracharge_knife, $id]);
    $error_message = $executer->error;
    
    $shipping_knife_cost = 0;
    $income_knife = 0;
    
    if(empty($error_message)) {
        $calculation = CalculationSelfAdhesive::Create($id);
        
        if($calculation instanceof CalculationBase) {
            $shipping_knife_cost = $calculation->shipping_knife_cost;
            $income_knife = $calculation->income_knife;
        }
        else {
            $error_message = $calculation;
        }
    }
    
    if(empty($error_message)) {
        $sql = "update calculation_result set shipping_knife_cost = ?, income_knife = ? where calculation_id = ?";
        $executer = new Executer($sql, [$shipping_knife_cost, $income_knife, $id]);
        $error_message = $executer->error;
    }
    
    if(empty($error_message)) {
        $sql = "select shipping_knife_cost, income, income_per_unit, income_cliche, income_knife from calculation_result where calculation_id = ? order by id desc limit 1";
        $fetcher = new Fetcher($sql, [$id]);
        $error_message = $fetcher->error;
        
        if($row = $fetcher->Fetch()) {
            $result['shipping_knife_cost'] = DisplayNumber(floatval($row['shipping_knife_cost']), 0);
            $result['input_shipping_knife_cost'] = round(floatval($row['shipping_knife_cost'] ?? 0), 0);
            $result['income_knife'] = DisplayNumber(floatval($row['income_knife']), 0);
            $result['income_total'] = DisplayNumber(round(floatval($row['income'] ?? 0), 0) + round(floatval($row['income_cliche'] ?? 0), 0) + round(floatval($row['income_knife'] ?? 0), 0), 0);
        }
    }
    
    $result['error'] = $error_message;
}

echo json_encode($result);
?>
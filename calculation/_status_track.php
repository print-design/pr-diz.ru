...
foreach($plan_editions as $plan_edition) {
    switch ($plan_edition['work_id']) {
        case WORK_PRINTING:
            $editions_by_work[ORDER_STATUS_PLAN_PRINT] = $plan_edition['last_name'].' '.PRINTER_NAMES[$plan_edition['machine_id']].' '.(DateTime::createFromFormat('Y-m-d', $plan_edition['date'])->format('d.m.Y')).' '.SHIFT_NAMES[$plan_edition['shift']];
            break;
        case WORK_LAMINATION:
            $editions_by_work[ORDER_STATUS_PLAN_LAMINATE] = $plan_edition['last_name'].' '.LAMINATOR_NAMES[$plan_edition['machine_id']].' '.(DateTime::createFromFormat('Y-m-d', $plan_edition['date'])->format('d.m.Y')).' '.SHIFT_NAMES[$plan_edition['shift']];
            break;
        case WORK_CUTTING:
            $editions_by_work[ORDER_STATUS_PLAN_CUT] = $plan_edition['last_name'].' '.CUTTER_NAMES[$plan_edition['machine_id']].' '.(DateTime::createFromFormat('Y-m-d', $plan_edition['date'])->format('d.m.Y')).' '.SHIFT_NAMES[$plan_edition['shift']];
            break;
    }
}
...
<div>
    ...
    <div style="display: inline-block; vertical-align: top;">
        ...
        <span style="font-size: smaller; font-style: italic;"><?= key_exists($row['status_id'], $editions_by_work) ? " ".$editions_by_work[$row['status_id']] : "" ?></span>

...
if(!empty($id)) {
    ...
    for($i = 1; $i <= $calculation->ink_number; $i++) {
        ...
        $sheet->setCellValue("C$rowindex", "|= ".DisplayNumber($calculation->print_area, 5)." * ".DisplayNumber($calculation->GetInkExpense(get_object_vars($calculation)[$ink], get_object_vars($calculation)[$cmyk], get_object_vars($calculation)[$lacquer], $calculation->data_ink->c_expense, $calculation->data_ink->m_expense, $calculation->data_ink->y_expense, $calculation->data_ink->k_expense, $calculation->data_ink->panton_expense, $calculation->data_ink->white_expense, $calculation->data_ink->lacquer_glossy_expense, $calculation->data_ink->lacquer_matte_expense, $calculation->data_ink->lacquer_selective_expense), 5)." * ".DisplayNumber(get_object_vars($calculation)[$percent], 5)." / 1000 / 100");
        $sheet->setCellValue("D$rowindex", "=".$calculation->print_area."*".$calculation->GetInkExpense(get_object_vars($calculation)[$ink], get_object_vars($calculation)[$cmyk], get_object_vars($calculation)[$lacquer], $calculation->data_ink->c_expense, $calculation->data_ink->m_expense, $calculation->data_ink->y_expense, $calculation->data_ink->k_expense, $calculation->data_ink->panton_expense, $calculation->data_ink->white_expense, $calculation->data_ink->lacquer_glossy_expense, $calculation->data_ink->lacquer_matte_expense, $calculation->data_ink->lacquer_selective_expense)."*".get_object_vars($calculation)[$percent]."/1000/100");
    ...
    for($i = 1; $i <= $calculation->ink_run2_number; $i++) {
        ...
        $sheet->setCellValue('A'.(++$rowindex), "Цена 1 кг чистой краски $i Второй прогон, руб");
        ...
        $sheet->setCellValue('A'.(++$rowindex), "Цена 1 кг КраскаСмеси $i Второй прогон, руб");
        ...
        $sheet->setCellValue('A'.(++$rowindex), "Расход КраскаСмеси $i Второй прогон, кг");
        $sheet->setCellValue("B$rowindex", $calculation->ink_expenses_run2[$i]);
        $sheet->setCellValue("C$rowindex", "|= ".DisplayNumber($calculation->print_area, 5)." * ".DisplayNumber($calculation->GetInkExpense(get_object_vars($calculation)[$ink_run2], get_object_vars($calculation)[$cmyk_run2], get_object_vars($calculation)[$lacquer_run2], $calculation->data_ink->c_expense, $calculation->data_ink->m_expense, $calculation->data_ink->y_expense, $calculation->data_ink->k_expense, $calculation->data_ink->panton_expense, $calculation->data_ink->white_expense, $calculation->data_ink->lacquer_glossy_expense, $calculation->data_ink->lacquer_matte_expense, $calculation->data_ink->lacquer_selective_expense), 5)." * ".DisplayNumber(get_object_vars($calculation)[$percent_run2], 5)." / 1000 / 100");
        $sheet->setCellValue("D$rowindex", "=".$calculation->print_area."*".$calculation->GetInkExpense(get_object_vars($calculation)[$ink_run2], get_object_vars($calculation)[$cmyk_run2], get_object_vars($calculation)[$lacquer_run2], $calculation->data_ink->c_expense, $calculation->data_ink->m_expense, $calculation->data_ink->y_expense, $calculation->data_ink->k_expense, $calculation->data_ink->panton_expense, $calculation->data_ink->white_expense, $calculation->data_ink->lacquer_glossy_expense, $calculation->data_ink->lacquer_matte_expense, $calculation->data_ink->lacquer_selective_expense)."*".get_object_vars($calculation)[$percent_run2]."/1000/100");
        $sheet->setCellValue("E$rowindex", "площадь запечатки * расход КраскаСмеси за 1 м2 * процент краски $i / 1000 / 100");
        
        $sheet->setCellValue('A'.(++$rowindex), "Стоимость КраскаСмеси Второй прогон $i, руб");
        ...
        $sheet->setCellValue('A'.(++$rowindex), "М2 испарения растворителя чистая КраскаСмеси Второй прогон $i, м2");
        ...
        $sheet->setCellValue('A'.(++$rowindex), "Расход испарения растворителя КраскаСмеси Второй прогон $i, кг");
        ...
        $sheet->setCellValue('A'.(++$rowindex), "Расход (краска + растворитель на одну краску) КраскаСмеси $i Второй прогон, руб");
        ...
        $sheet->setCellValue('A'.(++$rowindex), "Стоимость КраскаСмеси $i финальная Второй прогон, руб");
        $sheet->setCellValue("B$rowindex", $calculation->ink_costs_final_run2[$i]);
        $sheet->setCellValue("C$rowindex", "|= ЕСЛИ(".DisplayNumber($calculation->ink_costs_mix_run2[$i], 5)." < ".DisplayNumber($calculation->data_ink->min_price_per_ink, 5)." ; ".DisplayNumber($calculation->data_ink->min_price_per_ink, 5)." ; ".DisplayNumber($calculation->ink_costs_mix_run2[$i], 5).")");

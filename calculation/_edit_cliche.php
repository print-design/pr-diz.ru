...
if(empty($error_message)) {
    $sql = "select(select count(id) FROM calculation_cliche WHERE name = '".CalculationBase::FLINT."') flint_used, (select count(id) FROM calculation_cliche WHERE name = '".CalculationBase::KODAK."') kodak_used, (select count(id) FROM calculation_cliche WHERE name = '".CalculationBase::OLD."') old_used";
    $fetcher = new Fetcher($sql);
    $error_message = $fetcher->error;

...
if(!empty($status_id)) {
    ...
    if(in_array($status_id, ORDER_STATUSES_WITH_METERS)) {
        echo "<div style='font-size: smaller;'><span class='text-nowrap'>".rtrim(rtrim(DisplayNumber(floatval($result_cut), 2), '0'), ',')."</span> ".($unit == KG ? "кг" : "шт")." из <span class='text-nowrap'>".DisplayNumber(floatval(empty($quantity_sum) ? $quantity : $quantity_sum), 0)."</span></div>";
    }

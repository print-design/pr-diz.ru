<?php
// Этот файл подключается из cut_source/find.php и utilized/find.php.
// Переменная $page_folder к этому моменту уже задана в файле-обёртке index.php.
...
<form class="form-inline ml-auto mr-3<?=$find_class ?>" method="get" id="find-form" action="<?=APPLICATION.'/'.$page_folder.'/' ?>">
    <div class="input-group input-group-sm<?=$group_class ?>" id="find-group">
        ...
        <?= filter_input(INPUT_GET, "find") ?>
            &nbsp;&nbsp;
            <a href="<?=APPLICATION.'/'.$page_folder.'/' ?>"><i class="fas fa-times" style="color: white;"></i></a>
        </div>

...
class GrafikShiftReadonly {
    public function __construct(DateTime $date, $shift, $shift_data, GrafikReadonly $grafik, $editions, $date_editions_count, $shift_editions_count) {
        $this->date = $date;
        $this->shift = $shift;
    ...
    function Show() {
        if(count($this->editions) == 0) {
            ...
            foreach($this->editions as $key => $value) {
                $edition = new GrafikEditionReadonly($this->date, $this->shift, $this->shift_data, $this->grafik, $key, $value, $this->date_editions_count, $this->shift_editions_count);
                $edition->Show();
            }
        }
    }

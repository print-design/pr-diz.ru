...
const ROLE_ACCOUNTANT = 14;

const ROLES = array(ROLE_ELECTROCARIST, ROLE_STOREKEEPER, ROLE_COLORIST, ROLE_MARKER, ROLE_MANAGER, ROLE_LAM_HEAD, ROLE_FLEXOPRINT_HEAD, ROLE_SCHEDULER, ROLE_AUDITOR, ROLE_CUTTER, ROLE_MANAGER_SENIOR, ROLE_TECHNOLOGIST, ROLE_PACKER, ROLE_ACCOUNTANT);
const ROLE_NAMES = array(ROLE_COLORIST => "colorist", ROLE_LAM_HEAD => "lam_head", ROLE_FLEXOPRINT_HEAD => "flexoprint_head", ROLE_TECHNOLOGIST => "technologist", ROLE_MANAGER => "manager", ROLE_STOREKEEPER => "storekeeper", ROLE_ELECTROCARIST => "electrocarist", ROLE_CUTTER => "cutter", ROLE_MARKER => "marker", ROLE_AUDITOR => "auditor", ROLE_MANAGER_SENIOR => "manager-senior", ROLE_SCHEDULER => "scheduler", ROLE_PACKER => "packer", ROLE_ACCOUNTANT => "accountant");
const ROLE_LOCAL_NAMES = array(ROLE_COLORIST => "Колорист", ROLE_LAM_HEAD => "Начальник участка ламинации и резки", ROLE_FLEXOPRINT_HEAD => "Начальник участка флексопечати", ROLE_TECHNOLOGIST => "Технолог", ROLE_MANAGER => "Менеджер", ROLE_STOREKEEPER => "Кладовщик", ROLE_ELECTROCARIST => "Карщик", ROLE_CUTTER => "Резчик раскрой", ROLE_MARKER => "Маркировщик", ROLE_AUDITOR => "Ревизор", ROLE_MANAGER_SENIOR => "Старший менеджер", ROLE_SCHEDULER => "Планировщик", ROLE_PACKER => "Упаковщица", ROLE_ACCOUNTANT => "Бухгалтер");
const ROLE_TWOFACTOR = array(ROLE_COLORIST => 0, ROLE_LAM_HEAD => 0, ROLE_FLEXOPRINT_HEAD => 0, ROLE_TECHNOLOGIST => 0, ROLE_MANAGER => 0, ROLE_STOREKEEPER => 0, ROLE_ELECTROCARIST => 0, ROLE_CUTTER => 0, ROLE_MARKER => 0, ROLE_AUDITOR => 0, ROLE_MANAGER_SENIOR => 0, ROLE_SCHEDULER => 0, ROLE_PACKER => 0, ROLE_ACCOUNTANT => 0);
//const ROLE_TWOFACTOR = array(ROLE_COLORIST => 0, ROLE_LAM_HEAD => 1, ROLE_FLEXOPRINT_HEAD => 1, ROLE_TECHNOLOGIST => 1, ROLE_MANAGER => 1, ROLE_STOREKEEPER => 1, ROLE_ELECTROCARIST => 0, ROLE_CUTTER => 0, ROLE_MARKER => 0, ROLE_AUDITOR => 0, ROLE_MANAGER_SENIOR => 1, ROLE_SCHEDULER => 0, ROLE_PACKER => 0, ROLE_ACCOUNTANT => 0);
...
const ORDER_STATUSES_CALCULATED = array(ORDER_STATUS_CALCULATION, ORDER_STATUS_TECHMAP);
...
const DEFECT_TYPE_OTHER = 1000;
const DEFECT_TYPES = array(DEFECT_TYPE_DELAMINATION, DEFECT_TYPE_PRILADKA, DEFECT_TYPE_WRONG_CMYK, DEFECT_TYPE_WRONG_PANTON, DEFECT_TYPE_TEXT_LOSS, DEFECT_TYPE_WRONG_BARCODE, DEFECT_TYPE_OTHER);
const DEFECT_TYPE_NAMES = array(DEFECT_TYPE_DELAMINATION => "Деламинация", DEFECT_TYPE_PRILADKA => "Попадание приладки в отгруженную продукцию", DEFECT_TYPE_WRONG_CMYK => "Неверный цвет CMYK", DEFECT_TYPE_WRONG_PANTON => "Неверный цвет пантона", DEFECT_TYPE_TEXT_LOSS => "Исчезновение текста", DEFECT_TYPE_WRONG_BARCODE => "Неправильный баркод", DEFECT_TYPE_OTHER => "Другое");

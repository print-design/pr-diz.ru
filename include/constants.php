...
const ROLES = array(ROLE_ELECTROCARIST, ROLE_STOREKEEPER, ROLE_MARKER, ROLE_MANAGER, ROLE_SCHEDULER, ROLE_AUDITOR, ROLE_CUTTER, ROLE_MANAGER_SENIOR, ROLE_TECHNOLOGIST, ROLE_PACKER);
const ROLE_NAMES = array(ROLE_TECHNOLOGIST => "technologist", ROLE_MANAGER => "manager", ROLE_STOREKEEPER => "storekeeper", ROLE_ELECTROCARIST => "electrocarist", ROLE_CUTTER => "cutter", ROLE_MARKER => "marker", ROLE_AUDITOR => "auditor", ROLE_MANAGER_SENIOR => "manager-senior", ROLE_SCHEDULER => "scheduler", ROLE_PACKER => "packer");
const ROLE_LOCAL_NAMES = array(ROLE_ELECTROCARIST => "Карщик", ROLE_STOREKEEPER => "Кладовщик", ROLE_MARKER => "Маркировщик", ROLE_MANAGER => "Менеджер", ROLE_SCHEDULER => "Планировщик", ROLE_AUDITOR => "Ревизор", ROLE_CUTTER => "Резчик раскрой", ROLE_MANAGER_SENIOR => "Старший менеджер", ROLE_TECHNOLOGIST => "Технолог", ROLE_PACKER => "Упаковщица");
const ROLE_TWOFACTOR = array(ROLE_ELECTROCARIST => 0, ROLE_STOREKEEPER => 0, ROLE_MARKER => 0, ROLE_MANAGER => 0, ROLE_SCHEDULER => 0, ROLE_AUDITOR => 0, ROLE_CUTTER => 0, ROLE_MANAGER_SENIOR => 0, ROLE_TECHNOLOGIST => 0, ROLE_PACKER => 0);
//const ROLE_TWOFACTOR = array(ROLE_ELECTROCARIST => 0, ROLE_STOREKEEPER => 1, ROLE_MARKER => 0, ROLE_MANAGER => 1, ROLE_SCHEDULER => 0, ROLE_AUDITOR => 0, ROLE_CUTTER => 0, ROLE_MANAGER_SENIOR => 1, ROLE_TECHNOLOGIST => 1, ROLE_PACKER => 0);
...
const ORDER_STATUS_CUT_FINISHED = 13; // Закончили резку
...
const ORDER_STATUS_TITLES = array(ORDER_STATUS_SHIPPED => "Отгружено", ORDER_STATUS_SHIP_READY => "Ждет отгрузки", ORDER_STATUS_IN_PRODUCTION => "Производят", ORDER_STATUS_IN_WORK => "В работе", ORDER_STATUS_CALCULATION => "Расчеты", ORDER_STATUS_NOT_IN_WORK => "Расчеты", ORDER_STATUS_DRAFT => "Черновики", ORDER_STATUS_TRASH => "Корзина");
const ORDER_STATUSES_IN_CUT = array(ORDER_STATUS_CUT_PRILADKA, ORDER_STATUS_CUTTING, ORDER_STATUS_CUT_FINISHED, ORDER_STATUS_PACK_READY, ORDER_STATUS_SHIP_READY, ORDER_STATUS_SHIPPED);
const ORDER_STATUSES_WITH_METERS = array(ORDER_STATUS_CUTTING, ORDER_STATUS_CUT_FINISHED, ORDER_STATUS_PACK_READY, ORDER_STATUS_SHIP_READY, ORDER_STATUS_SHIPPED);
const ORDER_STATUS_NAMES = array(ORDER_STATUS_DRAFT => "Черновик", ORDER_STATUS_CALCULATION => "Сделан расчёт", ORDER_STATUS_WAITING => "Ждём подтверждения", ORDER_STATUS_CONFIRMED => "Ждём постановки в план", ORDER_STATUS_REJECTED => "Отклонено", ORDER_STATUS_TECHMAP => "Составлена тех. карта", ORDER_STATUS_TRASH => "В корзине", ORDER_STATUS_PLAN_PRINT => "В плане печати", ORDER_STATUS_PLAN_LAMINATE => "В плане ламинации", ORDER_STATUS_PLAN_CUT => "В плане резки", ORDER_STATUS_CUT_PRILADKA => "Приладка на резке", ORDER_STATUS_CUTTING => "Режется", ORDER_STATUS_CUT_FINISHED => "Закончили резку", ORDER_STATUS_PACK_READY => "Готово к упаковке", ORDER_STATUS_SHIP_READY => "Ждёт отгрузки", ORDER_STATUS_SHIPPED => "Отгружено", ORDER_STATUS_CUT_REMOVED => "Сняли с резки");
const ORDER_STATUS_COLORS = array(ORDER_STATUS_DRAFT => "gray", ORDER_STATUS_CALCULATION => "steelblue", ORDER_STATUS_WAITING => "goldenrod", ORDER_STATUS_CONFIRMED => "mediumseagreen", ORDER_STATUS_REJECTED => "crimson", ORDER_STATUS_TECHMAP => "saddlebrown", ORDER_STATUS_TRASH => "black", ORDER_STATUS_PLAN_PRINT => "#9933ff", ORDER_STATUS_PLAN_LAMINATE => "#4d009a", ORDER_STATUS_PLAN_CUT => "#27004e", ORDER_STATUS_CUT_PRILADKA => "chocolate", ORDER_STATUS_CUTTING => "brown", ORDER_STATUS_CUT_FINISHED => "maroon", ORDER_STATUS_PACK_READY => "paleturquoise", ORDER_STATUS_SHIP_READY => "RosyBrown", ORDER_STATUS_SHIPPED => "chocolate", ORDER_STATUS_CUT_REMOVED => "crimson");
const ORDER_STATUS_ICONS = array(ORDER_STATUS_DRAFT => "fas fa-edit", ORDER_STATUS_CALCULATION => "fas fa-check", ORDER_STATUS_WAITING => "fas fa-clock", ORDER_STATUS_CONFIRMED => "fas fa-check-double", ORDER_STATUS_REJECTED => "fas fa-times-circle", ORDER_STATUS_TECHMAP => "fas fa-file", ORDER_STATUS_TRASH => "fas fa-trash-alt", ORDER_STATUS_PLAN_PRINT => "fas fa-print", ORDER_STATUS_PLAN_LAMINATE => "fas fa-layer-group", ORDER_STATUS_PLAN_CUT => "fas fa-cut", ORDER_STATUS_CUT_PRILADKA => "fas fa-sliders-h", ORDER_STATUS_CUTTING => "fas fa-cut", ORDER_STATUS_CUT_FINISHED => "fas fa-cut", ORDER_STATUS_PACK_READY => "fas fa-box", ORDER_STATUS_SHIP_READY => "fas fa-box", ORDER_STATUS_SHIPPED => "fas fa-box", ORDER_STATUS_CUT_REMOVED => "fas fa-times-circle");

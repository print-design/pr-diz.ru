<?php
// Этот файл подключается из cut_source/index.php и utilized/index.php.
// Перед подключением должны быть заданы переменные:
//   $page_status_id   -- ROLL_STATUS_CUT или ROLL_STATUS_UTILIZED
//   $page_title       -- заголовок страницы ("Раскроили" / "Сработанная пленка")
//   $page_date_label  -- подпись колонки с датой ("раскроя" / "срабатывания")
//   $page_folder      -- имя папки для ссылок ("cut_source" / "utilized")

include '../include/topscripts.php';

// Преобразуем строковый ключ в константу статуса -- делаем это здесь, а не в файле-обёртке,
// потому что константа ROLL_STATUS_* определяется только после подключения topscripts.php выше
$page_status_id = ($page_status_key == 'cut') ? ROLL_STATUS_CUT : ROLL_STATUS_UTILIZED;
...
// вместе с массивами параметров для prepared statements
...
if(null !== filter_input(INPUT_POST, 'delete-film-submit')) {
    ...
    switch($type) {
        case 'pallet_roll':
            $sql_history = "delete from pallet_roll_status_history where pallet_roll_id = ?";
            $sql = "delete from pallet_roll where id = ?";
            break;
        case 'roll':
            $sql_history = "delete from roll_status_history where roll_id = ?";
            $sql = "delete from roll where id = ?";
            break;
    }
    
    if(!empty($sql)) {
        $error_message = (new Executer($sql_history, [$id]))->error;
...
// Фильтр для данных
list($wherefindpallet, $paramspallet, $wherefindroll, $paramsroll) = BuildRollFilter($page_status_id);
...
$row = (new Fetcher($sql, array_merge($paramspallet, $paramsroll)))->Fetch();
$total_weight = $row[0];
?>
<!DOCTYPE html>
<html lang="ru">
        ...
        <div class="container-fluid">
            ...
            <table class="table table-hover" id="content_table">
                ...
                <tbody>
                    ...
                    $fetcher = new Fetcher($sql, array_merge($paramspallet, $paramsroll));
                    ...
                    $sql = "select 'pallet_roll' type, pr.id id, pr.pallet_id pallet_id, pr.ordinal ordinal, prsh.date timestamp, DATE_FORMAT(prsh.date, '%d.%m.%Y') date, f.name film, "
                            ...
                            . "left join (select * from roll_cell_history where id in (select max(id) from roll_cell_history group by roll_id)) rch on rch.roll_id = r.id "
                            . "$wherefindroll "
                            . "order by ".$orderby."timestamp desc limit ?, ?";
                    
                    $list_params = array_merge($paramspallet, $paramsroll, [$pager_skip, $pager_take]);
            ...
            <div class="modal-dialog modal-dialog-aside" role="document">
                <div class="modal-content" style="padding-left: 35px; padding-right: 35px; width: 521px;">
                    ...
                    <form method="get">
                        ...
                        <a href="<?=APPLICATION ?>/<?=$page_folder ?>/" type="button" class="btn" name="filter_clear" style="margin-top: 20px; margin-bottom: 35px; padding: 5px; border-radius: 8px; background-color: #E4E1ED;"><img src="../images/icons/white-times.svg" />&nbsp;&nbsp;Очистить</a>

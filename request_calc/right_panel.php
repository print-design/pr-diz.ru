...
if(!empty ($id) && !empty ($date)) {
    ...
    if(!empty($lamination1_individual_density)) {
        $c_weight_lam1 = $lamination1_individual_density;
    }
    ...
    if(!empty($lamination2_individual_density)) {
        $c_weight_lam2 = $lamination2_individual_density;
    }
    ...
    $ink_k_expense = null;
    $ink_white = null;
    $ink_white_expense = null;
    $ink_panton = null;
    $ink_panton_expense = null;
    $ink_lacquer = null;
    $ink_lacquer_expense = null;
    $ink_ink_solvent = null;
    $ink_solvent_etoxipropanol = null;
    $ink_solvent_flexol82 = null;
    $ink_lacquer_solvent = null;
    $ink_min_price = null;
    ...
    $sql = "select c, c_currency, c_expense, m, m_currency, m_expense, y, y_currency, y_expense, k, k_currency, k_expense, white, white_currency, white_expense, panton, panton_currency, panton_expense, lacquer, lacquer_currency, lacquer_expense, ink_solvent, solvent_etoxipropanol, solvent_etoxipropanol_currency, solvent_flexol82, solvent_flexol82_currency, lacquer_solvent, min_price "
            . "from norm_ink where date <= '$date' order by date desc limit 1";
    $fetcher = new Fetcher($sql);
    if($row = $fetcher->Fetch()) {
        $ink_c = $row['c'];
...
<div id="calculation">
    ...
    <div class="row">
        <div class="col-4">
            <p class="param-title">Готовая продукция</p>
        ...
        <div class="col-4">
            <p class="param-title">Ламинация</p>
    ...
    <div id="costs" class="d-none" style="word-break: break-all; word-wrap: break-word;">
        ...
        <div class="param-name">Площадь тиража чистая, м<sup>2</sup></div>
        ...
        <?php if($unit == 'kg'): ?>
        <div class="param-value mb-2"><?="1000 * $quantity / (".$c_weight.(empty($c_weight_lam1) ? "" : " + ".$c_weight_lam1).(empty($c_weight_lam2) ? "" : " + ".$c_weight_lam2).") = ".$pure_area ?></div>
        <?php elseif($unit == 'pieces'): ?>
        <div class="param-value mb-2"><?="$stream_width / 1000 * $label_length / 1000 * $quantity = $pure_area" ?></div>
        <?php endif; ?>
            
        <div class="param-name">Ширина тиража обрезная, мм</div>
        <div class="algorithm">ширина ручья * количество ручьёв</div>
        <div class="param-value mb-2"><?="$stream_width * $streams_number = $pure_width" ?></div>
            
        <div class="param-name">Длина тиража чистая, м</div>
        <div class="algorithm">площадь тиража чистая / ширина тиража обрезная * 1000</div>
        <div class="param-value mb-2"><?="$pure_area / $pure_width * 1000 = $pure_length" ?></div>
            
        <?php if(!empty($lamination1_brand_name)): ?>
        <div class="param-name">Длина тиража чистая с ламинацией, м</div>
        <div class="algorithm">длина тиража чистая * (процент отходов для ламинатора + 100) / 100</div>
        <div class="param-value mb-2"><?="$pure_length * ($laminator_tuning_waste_percent + 100) / 100 = $pure_length_lam" ?></div>
        <?php endif; ?>
            
        <div class="param-name">Длина тиража с отходами, м</div>
        ...
        <div class="param-value mb-2"><?="$pure_length + ($pure_length * $tuning_waste_percents[$machine_id] / 100 + $tuning_lengths[$machine_id] * $ink_number) = $dirty_length" ?></div>
        <?php elseif(!empty ($lamination1_brand_name) && !empty ($pure_length_lam) && !empty ($laminator_tuning_length)): ?>
        <div class="param-value mb-2"><?="$pure_length_lam + $laminator_tuning_length = $dirty_length" ?></div>
        <?php endif; ?>
            
        <div class="param-name">Ширина тиража с отходами, мм</div>
        ...
        <?php if($no_ski): ?>
        <div class="param-value mb-2"><?="$pure_width = $dirty_width" ?></div>
        <?php else: ?>
        <div class="param-value mb-2"><?="$pure_width + $ski_width = $dirty_width" ?></div>
        <?php endif; ?>
            
        <div class="param-name">Площадь тиража с отходами, м<sup>2</sup></div>
        <div class="algorithm">длина тиража с отходами * ширина тиража с отходами / 1000</div>
        <div class="param-value mb-2"><?="$dirty_length * $dirty_width / 1000 = $dirty_area" ?></div>
            
        <div class="param-name">Вес материала печати чистый, кг</div>
        <div class="algorithm">площадь тиража чистая * удельный вес материала / 1000</div>
        <div class="param-value mb-2"><?="$pure_area * $c_weight / 1000 = $pure_weight" ?></div>
            
        <div class="param-name">Вес материала печати с отходами, кг</div>
        <div class="algorithm">площадь тиража с отходами * удельный вес материала / 1000</div>
        <div class="param-value mb-2"><?="$dirty_area * $c_weight / 1000 = $dirty_weight" ?></div>
            
        <div class="param-name">Стоимость материала печати, руб</div>
        ...
        <div class="param-value mb-2">0</div>
        <?php else: ?>
        <div class="param-value mb-2"><?="$dirty_weight * $c_price = $material_price" ?></div>
        <?php endif; ?>
            
        <!-------------------------------------------------------------------->
            
        <?php if($work_type_id == 2): ?>
        <div class="param-name">Время печати тиража без приладки, ч</div>
        <div class="algorithm">длина тиража чистая / 1000 / скорость работы флекс машины</div>
        <div class="param-value mb-2"><?="$pure_length / 1000 / $machine_speeds[$machine_id] = $print_time" ?></div>
            
        <div class="param-name">Время приладки, ч</div>
        <div class="algorithm">время приладки каждой краски / 60 * число красок</div>
        <div class="param-value mb-2"><?="$tuning_times[$machine_id] / 60 * $ink_number = $tuning_time" ?></div>
            
        <div class="param-name">Время печати с приладкой, ч</div>
        <div class="algorithm">время печати + время приладки</div>
        <div class="param-value mb-2"><?="$print_time + $tuning_time = $print_tuning_time" ?></div>
            
        <div class="param-name">Стоимость печати, руб</div>
        <div class="algorithm">время печати с приладкой * стоимость работы машины</div>
        <div class="param-value mb-2"><?="$print_tuning_time * $machine_prices[$machine_id] = $print_price" ?></div>
            
        <div class="param-name">Площадь печатной формы, м<sup>2</sup></div>
        <div class="algorithm">(припуск * 2 + ширина тиража с отходами / 1000 * 100) * (припуск * 2 + рапорт вала / 10)</div>
        <div class="param-value mb-2"><?="($cliche_additional_size * 2 + $dirty_width / 1000 * 100) * ($cliche_additional_size * 2 + $raport / 10) = $cliche_area" ?></div>
            
        <div class="param-name">Стоимость 1 печатной формы Флинт, руб</div>
        <div class="algorithm">площадь печатной формы * стоимость 1 см2 формы</div>
        <div class="param-value mb-2"><?="$cliche_area * $cliche_flint = $cliche_flint_price" ?></div>
            
        <div class="param-name">Стоимость 1 печатной формы Кодак, руб</div>
        <div class="algorithm">площадь печатной формы * стоимость 1 см2 формы</div>
        <div class="param-value mb-2"><?="$cliche_area * $cliche_kodak = $cliche_kodak_price" ?></div>
            
        <div class="param-name">Стоимость 1 печатной формы Тверь, руб</div>
        ...
        <div class="param-value mb-2"><?="$cliche_area * ($cliche_tver + $cliche_film * $cliche_tver_coeff) = $cliche_tver_price" ?></div>
            
        <div class="param-name">Стоимость комплекта печатных форм, руб</div>
        ...
        for($i=1; $i<=8; $i++) {
            if(!empty($ink_number) && $ink_number >= $i) {
                ...
                if(!empty($$ink_var)) {
                    if($$cliche_var == 'old') {
                        echo "<div class='param-value mb-2'>$cliche_price + 0 = ".$cliche_price."</div>";
                    }
                    elseif($$cliche_var == 'flint') {
                        echo "<div class='param-value mb-2'>$cliche_price + $cliche_flint_price = ".($cliche_price + $cliche_flint_price)."</div>";
                        $cliche_price += $cliche_flint_price;
                    }
                    elseif($$cliche_var == 'kodak') {
                        echo "<div class='param-value mb-2'>$cliche_price + $cliche_kodak_price = ".($cliche_price + $cliche_kodak_price)."</div>";
                        $cliche_price += $cliche_kodak_price;
                    }
                    elseif($$cliche_var == 'tver') {
                        echo "<div class='param-value mb-2'>$cliche_price + $cliche_tver_price = ".($cliche_price + $cliche_tver_price)."</div>";
        ...
        <div class="param-name">Стоимость скотча для наклейки форм, руб</div>
        <div class="algorithm">стоимость скотча для наклейки форм * число красок * площадь печатной формы / 10000</div>
        <div class="param-value mb-2"><?=($cliche_scotch ?? 0)." * ".($ink_number ?? 0)." * ".($cliche_area ?? 0)." / 10000 = ".(($cliche_scotch ?? 0) * ($ink_number ?? 0) * ($cliche_area ?? 0) / 10000) ?></div>
            
        <div class="param-name">Стоимость краски + лака + растворителя, руб</div>
        ...
        for($i=1; $i<=8; $i++) {
            if(!empty($ink_number) && $ink_number >= $i) {
                ...
                if(!empty($$ink_var)) {
                    ...
                    echo "<div class='param-value mb-2'>1) $dirty_area * ".$$percent_var." / 100 = $ink_area</div>";
                    ...
                    echo "<div class='param-value mb-2'>2) $ink_area * $ink_expense_final / 1000 = $ink_quantity</div>";
                    
                    // Стоимость неразведённой краски, руб
                    // количество краски * стоимость краски за 1 кг
                    $ink_price_sum = $ink_quantity * $ink_price_final;
                    echo "<div class='param-value mb-2'>3) $ink_quantity * $ink_price_final = $ink_price_sum</div>";
                    ...
                    echo "<div class='param-value mb-2'>4) $ink_quantity * $solvent_price_final = $solvent_price_sum</div>";
                    ...
                    echo "<div class='param-value mb-2'>5) ($ink_price_sum * $ink_solvent_final / 100) + ($solvent_price_sum * (100 - $ink_solvent_final) / 100) = $ink_solvent_price_sum</div>";
                    
                    echo "<div class='param-value mb-2'>$ink_solvent_price_sum + $ink_price = ".($ink_solvent_price_sum + $ink_price)."</div><div>---</div>";
        ...
        <?php if(!empty($lamination1_brand_name)): ?>
        <div class="param-name">Удельная стоимость клеевого раствора, руб/кг</div>
        ...
        <div class="param-value mb-2"><?="($glue_price * $glue_glue_part / ($glue_glue_part + $glue_solvent_part)) + ($glue_solvent_price * $glue_solvent_part / ($glue_glue_part + $glue_solvent_part)) = $glue_solvent_g" ?></div>
            
        <div class="param-name">Вес материала ламинации 1 чистый, кг</div>
        <div class="algorithm">площадь тиража чистая * удельный вес ламинации 1 / 1000</div>
        <div class="param-value mb-2"><?="$pure_area * $c_weight_lam1 / 1000 = $pure_weight_lam1" ?></div>
            
        <div class="param-name">Вес материала ламинации 1 с отходами, кг</div>
        ...
        <div class="param-value mb-2"><?="($pure_length_lam + $laminator_tuning_length) * $dirty_width / 1000 * $c_weight_lam1 / 1000 = $dirty_weight_lam1" ?></div>
            
        <div class="param-name">Стоимость материала ламинации 1, руб</div>
        <div class="algorithm">если сырьё заказчика: 0</div>
        ...
        <div class="param-value mb-2">0</div>
        <?php else: ?>
        <div class="param-value mb-2"><?="$c_price_lam1 * $dirty_weight_lam1 = $price_lam1_material" ?></div>
        <?php endif; ?>
            
        <div class="param-name">Стоимость клеевого раствора 1, руб</div>
        ...
        <div class="param-name">Стоимость процесса ламинации 1, руб</div>
        ...
        <div class="param-value mb-2"><?="$laminator_price + ($pure_length_lam / 1000 / $laminator_speed) * $laminator_price = $price_lam1_work" ?></div>
            
        <?php if(!empty($lamination2_brand_name)): ?>
        <div class="param-name">Вес материала ламинации 2 чистый, кг</div>
        <div class="algorithm">площадь тиража чистая * удельный вес ламинации 1 / 1000</div>
        <div class="param-value mb-2"><?="$pure_area * $c_weight_lam2 / 1000 = $pure_weight_lam2" ?></div>
            
        <div class="param-name">Вес материала ламинации 2 с отходами 2, кг</div>
        <div class="algorithm">(длина тиража с ламинацией + длина материала для приладки при ламинации)</div>
        <div class="algorithm">* ширина тиража с отходами / 1000 * удельный вес ламинации 1 / 1000</div>
        <div class="param-value mb-2"><?="($pure_length_lam + $laminator_tuning_length) * $dirty_width / 1000 * $c_weight_lam2 / 1000 = $dirty_weight_lam2" ?></div>
            
        <div class="param-name">Стоимость материала ламинации 2, руб</div>
        <div class="algorithm">если сырьё заказчика: 0</div>
        <div class="algorithm">иначе: удельная стоимость материала ламинации * вес материала с отходами</div>
        <?php if($lamination2_customers_material): ?>
        <div class="param-value mb-2">0</div>
        <?php else: ?>
        <div class="param-value mb-2"><?="$c_price_lam2 * $dirty_weight_lam2 = $price_lam2_material" ?></div>
        <?php endif; ?>
            
        <div class="param-name">Стоимость клеевого раствора 2, руб</div>
        <div class="algorithm">если марка плёнки начинается на pet:</div>
        <div class="algorithm">удельная стоимость клеевого раствора / 1000 * расход клея для ламинации ПЭТ</div>
        <div class="algorithm">* (чистая длина с ламинацией * ширина вала / 1000 + длина материала для приладки при ламинации)</div>
        <div class="algorithm">иначе:</div>
        ...
        <div class="param-name">Стоимость процесса ламинации 2, руб</div>
        <div class="algorithm">стоимость работы оборудования</div>
        <div class="algorithm">+ (длина чистая с ламинацией / 1000 / скорость работы оборудования) * стоимость работы оборудования</div>
        <div class="param-value mb-2"><?="$laminator_price + ($pure_length_lam / 1000 / $laminator_speed) * $laminator_price = $price_lam2_work" ?></div>
        <?php endif; ?>
            
        <div class="param-name">Итого себестоимость ламинации, руб</div>
        <div class="algorithm">материал1 + материал2 + клей1 + клей2 + процесс1 + процесс2</div>
        <div class="param-value mb-2"><?=($price_lam1_material ?? 0)." + ".($price_lam2_material ?? 0)." + ".($price_lam1_glue ?? 0)." + ".($price_lam2_glue ?? 0)." + ".($price_lam1_work ?? 0)." + ".($price_lam2_work ?? 0)." = ".($price_lam_total ?? 0) ?></div>
        <?php endif; ?>
        ...
        <div class="param-name">Итого себестоимость без форм, руб</div>
        ...
        <div class="param-value mb-2"><?=($material_price ?? 0)." + ".($print_price ?? 0)." + ".($ink_price ?? 0)." + ".($price_lam_total ?? 0)." + (".($cliche_scotch ?? 0)." * ".($ink_number ?? 0)." * ".($cliche_area ?? 0)." / 10000) = $cost_no_cliche" ?></div>
            
        <div class="param-name">Итого себестоимость с формами, руб</div>
        <div class="algorithm">итого стоимость без форм + стоимость комплекта печатных форм</div>
        <div class="param-value mb-2"><?=($cost_no_cliche ?? 0)." + ".($cliche_price ?? 0)." = $cost_with_cliche" ?></div>
            
        <?php if($unit == "kg" && !empty($quantity)): ?>
        <div class="param-name">Итого себестоимость за 1 кг без форм, руб</div>
        <div class="algorithm">итого себестоимость без форм / объём заказа</div>
        <div class="param-value mb-2"><?=($cost_no_cliche ?? 0)." / ".($quantity ?? 0)." = $cost_no_cliche_kg" ?></div>
            
        <div class="param-name">Итого себестоимость за 1 кг с формами, руб</div>
        <div class="algorithm">итого стоимость с формами / объём заказа</div>
        <div class="param-value mb-2"><?=($cost_with_cliche ?? 0)." / ".($quantity ?? 0)." = $cost_with_cliche_kg" ?></div>
        <?php endif; ?>
            
        <?php if($unit == "pieces" && !empty($quantity)): ?>
        <div class="param-name">Итого себестоимость за 1 шт без форм, руб</div>
        <div class="algorithm">итого себестоимость без форм / объём заказа</div>
        <div class="param-value mb-2"><?=($cost_no_cliche ?? 0)." / ".($quantity ?? 0)." = $cost_no_cliche_pieces" ?></div>
            
        <div class="param-name">Итого себестоимость за 1 шт с формами, руб</div>
        <div class="algorithm">итого стоимость с формами / объём заказа</div>
        <div class="param-value mb-2"><?=($cost_with_cliche ?? 0)." / ".($quantity ?? 0)." = $cost_with_cliche_pieces" ?></div>
        <?php endif; ?>
        
        <div class="param-name">Отгрузочная стоимость без форм, руб</div>
        <div class="algorithm">себестоимость без форм  + (себестоимость без форм * наценка / 100)</div>
        <div class="param-value mb-2"><?=$cost_no_cliche." + (".$cost_no_cliche. " * ".$extracharge." / 100) = ".($cost_no_cliche + ($cost_no_cliche * $extracharge / 100)) ?></div>
        
        <div class="param-name">Отгрузочная стоимость с формами, руб</div>
        <div class="algorithm">себестоимость с формами  + (себестоимость с формами * наценка / 100)</div>
        <div class="param-value mb-2"><?=$cost_with_cliche." + (".$cost_with_cliche. " * ".$extracharge." / 100) = ".($cost_with_cliche + ($cost_with_cliche * $extracharge / 100)) ?></div>
        
        <?php if($unit == "kg" && !empty($quantity)): ?>
        <div class="param-name">Отгрузочная стоимость за 1 кг без форм, руб</div>
        <div class="algorithm">себестоимость за 1 кг без форм  + (себестоимость за 1 кг без форм * наценка / 100)</div>
        <div class="param-value mb-2"><?=$cost_no_cliche_kg." + (".$cost_no_cliche_kg. " * ".$extracharge." / 100) = ".($cost_no_cliche_kg + ($cost_no_cliche_kg * $extracharge / 100)) ?></div>
        
        <div class="param-name">Отгрузочная стоимость с формами, руб</div>
        <div class="algorithm">себестоимость с формами  + (себестоимость с формами * наценка / 100)</div>
        <div class="param-value mb-2"><?=$cost_with_cliche_kg." + (".$cost_with_cliche_kg. " * ".$extracharge." / 100) = ".($cost_with_cliche_kg + ($cost_with_cliche_kg * $extracharge / 100)) ?></div>
        <?php endif; ?>
        
        <?php if($unit == "pieces" && !empty($quantity)): ?>
        <div class="param-name">Отгрузочная стоимость за 1 кг без форм, руб</div>
        <div class="algorithm">себестоимость за 1 кг без форм  + (себестоимость за 1 кг без форм * наценка / 100)</div>
        <div class="param-value mb-2"><?=$cost_no_cliche_pieces." + (".$cost_no_cliche_pieces. " * ".$extracharge." / 100) = ".($cost_no_cliche_pieces + ($cost_no_cliche_pieces * $extracharge / 100)) ?></div>
        
        <div class="param-name">Отгрузочная стоимость с формами, руб</div>
        <div class="algorithm">себестоимость за 1 кг с формами  + (себестоимость за 1 кг с формами * наценка / 100)</div>
        <div class="param-value mb-2"><?=$cost_with_cliche_pieces." + (".$cost_with_cliche_pieces. " * ".$extracharge." / 100) = ".($cost_with_cliche_pieces + ($cost_with_cliche_pieces * $extracharge / 100)) ?></div>
        <?php endif; ?>
    </div>
    <?php
    endif;

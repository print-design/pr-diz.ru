...
if($shift == 'day') {
    ...
    $sql = "select greatest(ifnull((select max(position) from plan_edition where work_id = $work_id and machine_id = $machine_id and shift = 'night' and date = date_add('$date', interval -1 day)), 0), "
            . "ifnull((select count(pc.id) from plan_continuation pc inner join plan_edition pe on pc.plan_edition_id = pe.id where pe.work_id = $work_id and pe.machine_id = $machine_id and pc.shift = 'night' and pc.date = date_add('$date', interval -1 day)), 0), "
            . "ifnull((select max(position) from plan_event where in_plan = 1 and work_id = $work_id and machine_id = $machine_id and shift = 'night' and date = date_add('$date', interval -1 day)), 0), "
            . "ifnull((select max(position) from plan_part where in_plan = 1 and work_id = $work_id and machine_id = $machine_id and shift = 'night' and date = date_add('$date', interval -1 day)), 0), "
            . "ifnull((select count(ppc.id) from plan_part_continuation ppc inner join plan_part pp on ppc.plan_part_id = pp.id where pp.work_id = $work_id and pp.machine_id = $machine_id and ppc.shift = 'night' and ppc.date = date_add('$date', interval -1 day)), 0))";
    $fetcher = new Fetcher($sql);
    if($row = $fetcher->Fetch()) {
        $max_value = $row[0];
    }
    
    $sql = "update plan_edition set position = ifnull(position, 0) + $max_value where work_id = $work_id and machine_id = $machine_id and date = '$date' and shift = '$shift'";
    $executer = new Executer($sql);
    $error = $executer->error;
    
    if(empty($error)) {
        $sql = "update plan_event set position = ifnull(position, 0) + $max_value where in_plan = 1 and work_id = $work_id and machine_id = $machine_id and date = '$date' and shift = '$shift'";
        $executer = new Executer($sql);
        $error = $executer->error;
    }
    
    if(empty($error)) {
        $sql = "update plan_part set position = ifnull(position, 0) + $max_value where in_plan = 1 and work_id = $work_id and machine_id = $machine_id and date = '$date' and shift = '$shift'";
        $executer = new Executer($sql);
        $error = $executer->error;
    }
}
elseif($shift == 'night') {
    ...
    $sql = "select greatest(ifnull((select max(position) from plan_edition where work_id = $work_id and machine_id = $machine_id and shift = 'day' and date = '$date'), 0), "
            . "ifnull((select count(pc.id) from plan_continuation pc inner join plan_edition pe on pc.plan_edition_id = pe.id where pe.work_id = $work_id and pe.machine_id = $machine_id and pc.shift = 'day' and pc.date = '$date'), 0), "
            . "ifnull((select max(position) from plan_event where in_plan = 1 and work_id = $work_id and machine_id = $machine_id and shift = 'day' and date = '$date'), 0), "
            . "ifnull((select max(position) from plan_part where in_plan = 1 and work_id = $work_id and machine_id = $machine_id and shift = 'day' and date = '$date'), 0), "
            . "ifnull((select count(ppc.id) from plan_part_continuation ppc inner join plan_part pp on ppc.plan_part_id = pp.id where pp.work_id = $work_id and pp.machine_id = $machine_id and ppc.shift = 'day' and ppc.date = '$date'), 0))";
    $fetcher = new Fetcher($sql);
    if($row = $fetcher->Fetch()) {
        $max_value = $row[0];
    }
